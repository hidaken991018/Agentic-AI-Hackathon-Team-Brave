...
export const hearingBaseSchema = z.object({
  ...
  economic_assumption: z.object({
    inflationRate: z
      .number()
      .describe("経済前提 > インフレ率（小数形式: 0.02 = 2%）"),
    investmentReturnRate: z
      .number()
      .describe("経済前提 > 投資利回り（小数形式: 0.04 = 4%）"),
  }),
  income: z.object({
    user: z.object({
      stableIncomeList: z
        .array(
          z.object({
            ...
            initialAmount: z
              .number()
              .describe("収入 > 継続収入初期値（本人）（万円）"),
            growthRate: z
              .number()
              .describe("収入 > 継続収入成長率（本人）（小数形式: 0.02 = 2%）"),
      ...
      stableIncomeList: z
        .array(
          z.object({
            ...
            initialAmount: z
              .number()
              .describe("収入 > 継続収入初期値（配偶者）（万円）"),
            growthRate: z
              .number()
              .describe(
                "収入 > 継続収入成長率（配偶者）（小数形式: 0.02 = 2%）",
              ),
  ...
  expenses: z.object({
    currentMonthlyLivingCost: z.number().describe("支出 > 月間生活費（万円）"),
    ...
    currentHousingCost: z.number().describe("支出 > 現在の月額住宅費（万円）"),
    insuranceList: z
      .array(
        z.object({
    ...
    lifeEventList: z
      .array(
        z.object({
          ...
          amount: z.number().describe("支出 > ライフイベント > 支出額（万円）"),
        }),
      )
  ...
  assets: z.object({
    planedInvestments: z.object({
      monthlyAmount: z
        .number()
        .describe("資産 > 計画資産投入額 > 月あたり投入額（万円）"),
      growthRate: z
        .number()
        .describe(
          "資産 > 計画資産投入額 > 投入額の増加率（小数形式: 0.02 = 2%）",
        ),
      endAge: z.number().describe("資産 > 計画貯蓄 > 貯蓄終了年齢"),
    }),
    currentCash: z.number().describe("資産 > 初期現金（万円）"),
    planedSaving: z.object({
      currentMonthlyAmount: z
        .number()
        .describe("資産 > 計画貯蓄 > 月あたり貯蓄額（万円）"),
      growthRate: z
        .number()
